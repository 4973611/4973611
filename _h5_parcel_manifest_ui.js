...
function shipParcelSpinner(){
    document.getElementById("btntextMan").style.display = 'none'
    document.getElementById("btnspinnerMan").style.display = ''
    setTimeout(shipParcels, 20)
}
function shipParcels(){
    let tempTable = document.getElementById('shipmentsTable')
    ...
    let sendData = {
        shipIds: shipIds,
        manifestId: document.getElementById("createManButton").getAttribute("data-value")
    }
    ...
    let suiteletURL = ns.url.resolveScript({
        scriptId: "customscript_h5_sl_manifest_parcels",
        deploymentId: "customdeploy_h5_sl_manifest_parcels",
        returnExternalUrl: true
    });
