...
/******************************************************* TRUCKLOAD *******************************************************************/
function saveCarriersSpinner(data){
    createWaiter()
    setTimeout(saveCarrier, 30, data)
}

function saveCarrier(data){
    console.log("save carrier", data)

    let carriers = []
    for(x=0; x<data.length; x++){
        carriers.push({
            shipId: data[x],
            carrier: document.getElementById("carrier" + data[x]).value,
            cost: document.getElementById("totalCost" + data[x]).value,
            quoteNum: document.getElementById("quoteNum" + data[x]).value
        })
    }
    ...
    let suiteletURL = ns.url.resolveScript({
        scriptId: "customscript_h5_sl_save_tl_carrier",
        deploymentId: "customdeploy_h5_sl_save_tl_carrier",
...
/******************************************************* MODALS *******************************************************************/
function openShipments(data){

    console.log("shipmentId", data)

    for(x=0; x<data.length; x++){
        window.open("https://4973611-sb1.app.netsuite.com/app/common/custom/custrecordentry.nl?rectype=1977&id=" +  data[x])
    }
}
/******************************************************* LTL MODAL *******************************************************************/
...
function getShipData(shipModelData){
    ...
    let suiteletURL = ns.url.resolveScript({
        scriptId: "customscript_h5_sl_get_ltl_ship_data_mod",
        deploymentId: "customdeploy_h5_sl_get_ltl_ship_data_mod",
        returnExternalUrl: true
    });
...
function getTLData(shipModelData){
    let shipId = shipModelData
    console.log("shipId", shipId)
    ...
    modalTLData = JSON.parse(response.body)

    buildTLShip(shipId)

}

function buildTLShip(shipId) {

    console.log("buildModal", shipId)
    let modalContainer = document.getElementById('modalTLContents')

    let carriers = '<option value=""></option>'
    for(z=0; z<modalTLData[0].carriers.length; z++){
        carriers += '<option value="' + modalTLData[0].carriers[z].carrierVal + '">' + modalTLData[0].carriers[z].carrierText + '</option>'
    }

    let modalContent = '<div class=modal-header style=padding-bottom:0;margin-bottom:0><h5 class=modal-title style=font-size:1vw;padding-bottom:0;margin-bottom:1%>' + modalTLData[0].shipName + '</h5><p id=shipmentID style=display:none>' + modalTLData[0].shipId + '</p><button class=close type=button data-bs-dismiss=modal aria-label=Close><span aria-hidden=true>×</span></button></div><div class=modal-body id=modalBody><div class=row style=height:auto;padding:.5%><table style="width:100%;border-radius:1px;margin-bottom:.5%;box-shadow:1px 2px 3px rgba(0,0,0,.2);background-color:#4d5f79"class=titleBars><tr><td style=font-size:.75vw;font-weight:700;padding-left:1%;padding:.5%;color:#fff>Parties</table><div class="col-6 container-fluid"style=padding:0 align=center><form action=""autocomplete=off class=form-horizontal id=shipAddressForm role=form><div class=form-group style=margin-right:.5%!important;margin-left:.5%!important><table style=width:100%;border-radius:1px;margin-bottom:1%;background-color:rgba(0,0,0,.2) class=titleBars><tr><td style=font-size:.75vw;font-weight:700;padding-left:1%;padding:.5%>SHIP DATE</table><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=dateSL style=font-size:.75vw>Date:</span> <input class=form-control-plaintext id=dateS placeholder=Date style=font-size:.75vw value="' + modalTLData[0].shipDate + '"type=date aria-describedby=basic-addon1 aria-label=Company></div><table style=width:100%;border-radius:1px;margin-bottom:1%;margin-top:1%;background-color:rgba(0,0,0,.2) class=titleBars><tr><td style=font-size:.75vw;font-weight:700;padding-left:1%;padding:.5%>SHIP FROM</table><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important;margin-right:.5%!important><span class=input-group-text id=locationSL style=font-size:.75vw>Location:</span> <input class=form-control-plaintext id=locationS placeholder=Location style=font-size:.75vw value="' + modalTLData[0].location + '"aria-describedby=basic-addon1 aria-label="Company Address"readonly></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=phoneSL style=font-size:.75vw><i class="fa-phone fa-solid"></i></span> <input class=form-control id=phoneS placeholder=Phone style=font-size:.75vw value="' + modalTLData[0].shipPhone + '"type=tel aria-describedby=basic-addon1 aria-label=Company></div><table style=width:100%;border-radius:1px;margin-bottom:1%;margin-top:1%;background-color:rgba(0,0,0,.2) class=titleBars><tr><td style=font-size:.75vw;font-weight:700;padding-left:1%;padding:.5%>BILL TO</table><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=addreBL style=font-size:.75vw;padding:.5%>Addre:</span> <input class=form-control id=addreB placeholder=State style=font-size:.75vw;padding:.5% value="' + modalTLData[0].billToAddre + '"type=tel></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=stateBL style=font-size:.75vw;padding:.5%>State:</span> <input class=form-control id=stateB placeholder=State style=font-size:.75vw;padding:.5% value="' + modalTLData[0].billToState + '"type=state></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=countryBL style=font-size:.75vw;padding:.5%>Country:</span> <input class=form-control id=countryB placeholder=Country style=font-size:.75vw;padding:.5% value="' + modalTLData[0].billToCountry + '"type=country></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=accountBL style=font-size:.75vw;padding:.5%>Account:</span> <input class=form-control id=accountB placeholder=Account style=font-size:.75vw;padding:.5% value="' + modalTLData[0].billToAccount + '"></div></div></form></div><div class="col-6 container-fluid"style=padding:0 align=center><form action=""autocomplete=off class=form-horizontal id=shipToAddressForm role=form><div class=form-group style=margin-right:.5%!important;margin-left:.5%!important><table style=width:100%;border-radius:1px;margin-bottom:1%;background-color:rgba(0,0,0,.2) class=titleBars><tr><td style=font-size:.75vw;font-weight:700;padding-left:1%;padding:.5%>SHIP TO</table><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=companyL style=font-size:.75vw;padding:.5%>Company:</span> <input class=form-control id=company placeholder=Company style=font-size:.75vw;padding:.5% value="' + modalTLData[0].conAddre + '"></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=attnL style=font-size:.75vw;padding:.5%>Attn:</span> <input class=form-control id=attn placeholder=Company style=font-size:.75vw;padding:.5% value="' + modalTLData[0].conAttn + '"></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=add1L style=font-size:.75vw;padding:.5%>Address 1:</span> <input class=form-control id=add1 placeholder="Address 1"style=font-size:.75vw;padding:.5% value="' + modalTLData[0].conAdd1 + '"></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=add2L style=font-size:.75vw;padding:.5%>Address 2:</span> <input class=form-control id=add2 placeholder="Address 2"style=font-size:.75vw;padding:.5% value="' + modalTLData[0].conAdd2 + '"></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=cityL style=font-size:.75vw;padding:.5%>City:</span> <input class=form-control id=city placeholder=City style=font-size:.75vw;padding:.5% value="' + modalTLData[0].conCity + '"></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=stateL style=font-size:.75vw;padding:.5%>State:</span> <input class=form-control id=state placeholder=State: style=font-size:.75vw;padding:.5% value="' + modalTLData[0].conState + '"></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=zipL style=font-size:.75vw;padding:.5%>Zip:</span> <input class=form-control id=zip placeholder=Zip style=font-size:.75vw;padding:.5% value="' + modalTLData[0].conZip + '"></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=countryL style=font-size:.75vw;padding:.5%>Country:</span> <input class=form-control id=country placeholder=Country style=font-size:.75vw;padding:.5% value="' + modalTLData[0].conCountry + '"></div><div class="input-group mb-3"style=padding:0!important;margin-bottom:1%!important;margin-top:0!important><span class=input-group-text id=contactL style=font-size:.75vw;padding:1%><i class="fa-phone fa-solid"></i></span> <input class=form-control id=phone placeholder=Phone style=font-size:.75vw;padding:.5% value="' + modalTLData[0].conPhone + '"></div></div></form></div></div>'

    modalContent +='<div class=row style=height:auto;padding:.5%></div><div class=modal-footer><p align="left" style="padding: 0%; margin: 0%; font-weight: bold">Carrier & Total:</p><br><select id="carrierVal" style="padding: .25%;">' + carriers + '</select>'

    modalContent += '<input id="totalCost" type="number" style="font-size: 1vw; border-color: gray !important;">'
    modalContent += '<input id="quoteNum" type="text" placeholder="Quote Number" style="font-size: .75vw;">'
    modalContent +='<button class="btn btn-secondary"type=button data-bs-dismiss=modal id=closeButton>Close</button><button class="btn btn-primary" type=button onclick=saveShipSpinner("save")><p id="saveText" style="padding: 0%; margin: 0%;">Save & Assign</p><i id="saveSpin" class="fa fa-spinner fa-spin" style="display: none"></i></button></div>'
...
function saveShipSpinner(action){
    if(action == 'save'){
        document.getElementById("saveText").style.display = "none"
        document.getElementById("saveSpin").style.display = ""
    }else{
        document.getElementById("saveRateText").style.display = "none"
        document.getElementById("saveRateSpin").style.display = ""
    }
    setTimeout(saveShipment, 50, action)
}
function saveShipment(action){
    ...
    let addressObj = {
        ...
        "carrier": document.getElementById("carrierVal").value,
        "totalCost": document.getElementById("totalCost").value,
        "quoteNum": document.getElementById("quoteNum").value,
        "action": action
    }
    ...
    var suiteletURL = ns.url.resolveScript({
        scriptId: "customscript_h5_sl_save_ship_ltl",
        deploymentId: "customdeploy_h5_sl_save_ship_ltl",
        returnExternalUrl: true
    });
