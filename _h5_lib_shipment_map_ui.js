...
function back(){
    let url = "https://4973611.app.netsuite.com/app/site/hosting/scriptlet.nl?script=7550&deploy=1"

    location.replace(url)
}


function renderMultiStop(){
    $("#mGroupButtonText").hide('fast');
    $("#mGroupSpinner").show('fast');

    var options = document.getElementById('subsidiaryMS').selectedOptions;
    console.log(options)
    var subsidiaryMS = Array.from(options).map(({value}) => value);
    let warehouseLocationMS = document.getElementById("warehouseLocationMS").value

    let params = {
        "warehouse": warehouseLocationMS,
        "subsidiaries": subsidiaryMS,
    }

    console.log(params)

    let mapUrl = "https://4973611.app.netsuite.com/app/site/hosting/scriptlet.nl?script=7551&deploy=1&warehouse=" + params.warehouse + "&subsidiaries=" + params.subsidiaries
...
function createStops() {
    ...
    // let urlX = "https://4973611.extforms.netsuite.com/app/site/hosting/scriptlet.nl?script=7472&deploy=1&compid=4973611&ns-at=AAEJ7tMQnJVBmXA2tJZFU70L7OtltsULYyeDwyb0xTXJ0_40Eac"
    //
    // let responseX = nlapiRequestURL(urlX, JSON.stringify(shipmentsToSend), headerObj, 'POST');
    // console.log(responseX)
    let response = ns.https.post({
        url: "https://4973611.extforms.netsuite.com/app/site/hosting/scriptlet.nl?script=7472&deploy=1&compid=4973611&ns-at=AAEJ7tMQnJVBmXA2tJZFU70L7OtltsULYyeDwyb0xTXJ0_40Eac",
        headers: headerObj,
        body: JSON.stringify(shipmentsToSend)
    })
    let temp = JSON.parse(responseX.body)
    if(temp.code == 201){
